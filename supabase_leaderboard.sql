...
-- Top users by reputation (products * 20 + prices * 5)
CREATE OR REPLACE FUNCTION leaderboard_reputation(p_limit integer DEFAULT 20)
...
LANGUAGE sql SECURITY DEFINER AS $$
    SELECT
        ...
        COALESCE(pc.cnt, 0)  AS products_count,
        COALESCE(prc.cnt, 0) AS prices_count,
        COALESCE(pc.cnt, 0) * 20 + COALESCE(prc.cnt, 0) * 5 AS reputation
    FROM (
